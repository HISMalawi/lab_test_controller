...
class PatientController < ApplicationController
    ...
    def search_by_name_results
        ...
        if params[:option] == "by_identifier"
            ...
            res = PatientService.scan_patient(ident,session[:user][0])

            if res[0] ==  true  
                res[1].each do |data|
                  
                    p_id = data['patient_id']            
                    f_name = data['person']['names'][0]['given_name']
                    s_name = data['person']['names'][0]['family_name']
                    birthdate = data['person']['birthdate']
                    gender = data['person']['gender']
                    city_village = data['person']['addresses'][0]['city_village']
                    state_province = data['person']['addresses'][0]['state_province']
                    identifier = ident
                    @found_patients.push([p_id,f_name,s_name,gender,birthdate,city_village,state_province,identifier])      
                end
            else
    ...
    def search_patient
        ...
        if search_option == "by_name"
            ...
            identifier = params[:identifier]
            redirect_to "/patient/search_by_name_results?identifier=#{identifier}&option=by_identifier"        
        end
    end
